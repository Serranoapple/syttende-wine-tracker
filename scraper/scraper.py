...
def wine_searcher_query(wine: dict) -> Optional[float]:
    ...
    """
    query_parts = [wine["producer"], wine["name"], str(wine["vintage"])]
    ...
    try:
        ...
        for category, url in live_urls.items():
            ...
            for wine in wines:
                ...
                if is_new:
                    ...
                    desc = (f"Ny vin tilføjet: {wine['vintage']} {wine['name']} "
                    ...
                    if old_price is not None and old_price != wine["price_dkk"]:
                        ...
                        desc = (f"{wine['vintage']} {wine['name']} ({wine['producer']}) "
        ...
        if last_run.data:
            ...
            for wine_id in newly_missing:
                ...
                if wine_info.data:
                    w = wine_info.data[0]
                    desc = f"Vin fjernet fra kortet: {w['vintage']} {w['name']} — {w['producer']}"
