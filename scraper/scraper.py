...
# Single lowercase letters that are legitimate parts of producer names
...
def clean_producer(producer: str) -> str:
    """
    Remove PDF line-break artefacts from producer names.
    pdfplumber sometimes appends a stray 1-2 char lowercase token from
    the next column/line when text reflows — e.g. "Bodegas Navera n".
    """
    ...
    if m:
        ...
        if token not in _PRODUCER_PARTICLES:
            producer = producer[:m.start()].strip()
    return producer
