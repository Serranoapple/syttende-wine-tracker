    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        for page in pdf.pages:
            ...
            for raw_line in lines:
                ...
                # Detect category
                # Only fire on lines that ARE the keyword (or close to it) —
                # not on wine entries that happen to contain e.g. "ROSÉ" in the name.
                # Guard: line must not end in a price (= not a wine entry)
                # AND the keyword must be a significant portion of the line.
