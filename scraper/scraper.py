...
# Explicit map from PDF section headers → category.
# Keys are substrings that appear in section-header lines (ALL CAPS in PDF).
# Order matters: more specific entries first.
SECTION_MAP = [
    # ── bobler PDF ──────────────────────────────────────────
    ("WINE BY THE GLASS",       None),          # subcategory, not a main category change
    ("SPARKLING WINE",          "sparkling"),
    ("CÔTEAUX CHAMPENOIS",      "still_champagne"),
    ("COTEAUX CHAMPENOIS",      "still_champagne"),
    ("FORTIFIED WINE",          "fortified"),
    ("SWEET WINE",              "dessert"),
    ("NON-ALCOHOLIC BEVERAGE",  "non_alcoholic"),
    ...
    ("HVIDVIN",                 "white"),
    # ── rødvin/rosé PDF ─────────────────────────────────────
    ("ROSÉ & RØDVIN",           "rose"),        # first page of that PDF
    ("RED WINE",                "red"),
    ("RØDVIN",                  "red"),
    ("ROSÉ",                    "rose"),
    # ── avec PDF ────────────────────────────────────────────
    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        for page in pdf.pages:
            ...
            for raw_line in lines:
                ...
                # Only fires on non-wine-entry lines (no trailing price).
                # lock_category=True for avec PDF (subsections must not override).
                _is_wine_entry = bool(re.search(r'\d{3,5}$', line))
                if not _is_wine_entry and not lock_category:
                    for kw, cat in SECTION_MAP:
                        if kw in upper and len(line) < 60:
                            if cat is not None:
